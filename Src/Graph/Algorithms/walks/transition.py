...
def parse_args():
    ...
    parser.add_argument('--input', nargs='?', default='data/c2b2rdf/train_pos.csv',
                        help='Input graph path')

    parser.add_argument('--output', nargs='?', default='matrix.txt',
                        help='store transition matrix')
    parser.add_argument('--edges-list', nargs='?', default='data/c2b2rdf/edges.txt',
                        help='edge list')
    parser.add_argument('--em-iteration', default=5, type=int,
                      help='EM iterations for transition matrix')

    parser.add_argument('--e-step', default=3, type=int,
...
def main(args): 
    ...
    num_edge_types = len(open(args.edges_list).readlines(  ))
    type_size = num_edge_types
    if args.directed:    
        type_size = (num_edge_types + 1) * 2
    trans_matrix = initialize_edge_type_matrix(type_size)

    print(trans_matrix)
    print("------begin to read graph---------") 
    G = read_graph(args.input,num_edge_types,args.weighted,args.directed)
    ...
    for i in range(args.em_iteration):
        ...
        print("Did %d walks!" % len(walks))
        print((str(i), "th iteration for Updating transition matrix!"))
        trans_matrix = update_trans_matrix(walks,type_size,args.e_step)#E step
