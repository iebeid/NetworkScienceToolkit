...
def preprocess_word(word, pp):
    preprocessed_word = word.lower()
    if pp:
        preprocessed_word, preprocessed_word_node_type = parse_word_node_type(preprocessed_word)
...
def parse_word_node_type(labeled_word):
    ...
    return pure_word, processed_node_type
                ...
                if (index != middle_index) and (context_word != None):
                    ...
                    for i in range(negative_sampling_window_size):
                        if pp:
                            for node_type_list in node_type_lists:
                                if context_word_node_type == node_type_list[0]:
                                    ...
                                    random_het_negative_index = random.sample(
                                        range(len(current_node_type_vocab_list) - 1), 1)
                                    chosen_random_word = current_node_type_vocab_list[random_het_negative_index]
                                    chosen_random_word_index = get_word_index(model_dataframe, chosen_random_word)
                                    ...
                                    negative_sample_index = heterogenous_model_dataframe[
                                        "cumulative_distribution"].searchsorted(chosen_random_word_index)
                            ...
                            random_negative_index = random.sample(range(vocab_size - 1), 1)
...
if __name__ == "__main__":
    ...
    model_data_frame = "model_dataframe.csv"
    if pp:
    ...
    train(corpus, window_size, embedding_dimension, number_of_epochs, learning_rate, min_learning_rate,
          negative_sampling_window_size, model_file, model_data_frame, pp, sample_rate, sample_threshold,
          factor_of_negative_sample)
    model = np.array([])
    with open(model_file, 'rb') as f:
        model = np.load(f, allow_pickle=True)
    model_dataframe = pd.read_csv(model_data_frame)
